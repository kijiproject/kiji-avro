...
/**
 * A wrapper around a Hadoop {@link org.apache.hadoop.io.SequenceFile} that also supports
 * reading and writing Avro data.
 *
 * <p>The vanilla Hadoop <code>SequenceFile</code> contains a <i>header</i> followed by a
 * sequence of <i>records</i>.  A <i>record</i> consists of a <i>key</i> and a
 * <i>value</i>.  The <i>key</i> and <i>value</i> must either:</p>
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>Since Avro data are Plain Old Java Objects (e.g., <code>Integer</code> for data with
 * schema <i>"int"</i>), they do not implement <i>Writable</i>.  Furthermore, a
 * {@link org.apache.hadoop.io.Serialization} implementation cannot determine whether an
 * object instance of type <code>CharSequence</code> that also implements
 * <code>Writable</code> should be serialized using Avro or WritableSerialization.</p>
 *
 ...
 * </ul>
 */
public final class AvroSequenceFile {
  ...
  public static SequenceFile.Writer createWriter(Writer.Options options) throws IOException {
    // TODO: This method is deprecated. We should construct a specialized Writer.Option instead.
    return SequenceFile.createWriter(options.getFileSystem(),
        ...
        options.getBlockSizeBytes(), options.getCompressionType(),
        options.getCompressionCodec(), options.getProgressable(),
